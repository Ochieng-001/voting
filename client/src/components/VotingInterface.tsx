...
export function VotingInterface() {
  ...
  const handleVote = async (candidateId: number, candidateName: string) => {
    ...
    try {
      ...
      toast({
        ...
        description: `Your vote for ${candidateName} has been recorded. Transaction: ${result.txHash.substring(0, 10)}...`,
      });
    } catch (error: any) {
      toast({
        title: "Voting Failed",
  ...
  const canVote = voterData?.isRegistered && !voterData?.hasVoted;
      ...
      <CardContent>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {candidates.map((candidate) => (
            <Card key={candidate.id} className="border border-gray-200 hover:shadow-lg transition-shadow">
              <CardContent className="p-6">
                <div className="text-center">
                  <img 
                    src={candidate.photo || `https://ui-avatars.com/api/?name=${encodeURIComponent(candidate.name)}&size=96&background=random`}
                    alt={candidate.name}
                    className="w-24 h-24 rounded-full mx-auto mb-4 object-cover border-4 border-gray-200"
                    onError={(e) => {
                      (e.target as HTMLImageElement).src = `https://ui-avatars.com/api/?name=${encodeURIComponent(candidate.name)}&size=96&background=random`;
                    }}
                  />
